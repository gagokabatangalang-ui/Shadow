...
// PORT - works on Replit, Glitch, Railway, Heroku, VPS
    ...
    try {
        ...
        const getRes = await fetch(`https://api.github.com/repos/${GITHUB_REPO}/contents/keys.json?ref=${GITHUB_BRANCH}`, {
            headers: { 
                ...
                'User-Agent': 'ShadowBot/1.0'
            }
        });
        
        if (!getRes.ok) {
            console.error('Failed to get file SHA:', await getRes.text());
            return false;
        }
        ...
        const putRes = await fetch(`https://api.github.com/repos/${GITHUB_REPO}/contents/keys.json`, {
            method: 'PUT',
            headers: { 
                'Authorization': `Bearer ${GITHUB_TOKEN}`, 
                'Accept': 'application/vnd.github.v3+json',
            ...
            body: JSON.stringify({
                message: `Update keys.json - ${new Date().toISOString()}`,
...
// KEEP ALIVE SERVER - fixes port issues on Replit/Glitch/Railway
const server = http.createServer((req, res) => {
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ 
        status: 'Shadow Bot Online', 
...
**Game:** Flick/Onetap
**Pushed to GitHub:** ${pushed ? '✅ Yes' : '❌ Failed'}
